...
export default function Exercise({ navigation }) {
    ...
    async function fetchData() {
        ...
        console.log("Endurance:", response);
        setEndurance(response);
    }

    return (
        <View style={{ flex: 2 }}>
            <Header title="Endurance" color="#F2994A" />
            <Text
                onLayout={() => {
                    fetchData();
                }}
...
async function submitForm(enu) {
    ...
    try {
        const response = await fetch(host_path + "/api/exercise/endurance", {
            method: "POST",
