...
export default function Exercise({ navigation }) {
    ...
    async function fetchData() {
        ...
        console.log("Flexibility:", response);
        setFlexibility(response);
    }

    return (
        <View style={{ flex: 2 }}>
            <Header title="Flexibility" color="#F2994A" />
...
async function submitForm(enu) {
    ...
    try {
        const response = await fetch(host_path + "/api/exercise/flexibility", {
            method: "POST",
            headers: await HeaderWithVeification(),
            body: JSON.stringify({
                totalMin: flexibility,
            }),
        });
